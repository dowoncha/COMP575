...
void Scene::SetupEyeTransform(const glm::vec3& u, const glm::vec3& v, const glm::vec3& w, const glm::vec3& p)
{
    ...
    EyeTransform[2] = glm::vec4(w, 0);

    // Homogenous coordinate magic
    glm::vec4 temp = EyeTransform * glm::vec4(p, 1.0f);
    EyeTransform[0][3] = -temp.x;
    EyeTransform[1][3] = -temp.y;
    EyeTransform[2][3] = -temp.z;
}
...
void Scene::SetupProjTransform(float l, float r, float b, float t, float n, float f)
{
  ...
  ProjTransform[1] = glm::vec4( 0, n2 / (r - 1), 0, (b+t)/(b-t), 0);
...
{
    ViewTransform[0] = glm::vec4(nx / 2, 0, 0, (nx - 1) / 2);
    ViewTransform[1] = glm::vec4(0, ny / 2, 0, (ny - 1) / 2);
    ViewTransform[2] = glm::vec4(0, 0, 1, 0);
